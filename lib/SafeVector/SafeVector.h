...
class SafeVector {
  public:
    SafeVector(size_t size = DEFAULT_SIZE, T *data = nullptr, bool strict = DEFAULT_STRICT) noexcept;
    ~SafeVector() noexcept;
    ...
    constexpr bool is_strict() const noexcept { return strict; };
    T *get_array() const noexcept { return data; };
  private:
